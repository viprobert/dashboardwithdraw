// src/services/vg98.service.ts
import api from "../utils/axios";
import siteData from "../data/sites.json";

const site: any = siteData.find((s) => s.id === 6);
if (!site) throw new Error("VG98 site config not found in sites.json");


let cachedToken: string | null = localStorage.getItem("vg98_token") || null;
...
async function loginAndGetToken(): Promise<string> {
  ...
  cachedToken = token;
  localStorage.setItem("vg98_token", token); 
  return token;
}
 ...
 * Fetch task counts
 */
export async function fetchVG98Counts(pageNo = 1, maxResult = 50) {
  try {
    ...
    console.log("🧮 Vg98 counts:", counts);
    return counts;
  } catch (error: any) {
    ...
    if (status === 401 || code === "INVALID_TOKEN") {
      ...
      localStorage.removeItem("vg98_token");
      cachedToken = null;
      const newToken = await loginAndGetToken();
      return await fetchVG98Counts(pageNo, maxResult);
    }

    console.error("🔴 fetchVG98Counts error:", error);
    throw error;
  }
}
